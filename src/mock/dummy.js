  ...
  <>
    {props.statusTicket === 1 ? (
      <div className="status status--use inline-flex ">
        <img src={`${use}`} alt="" />
        <button type="button" className="button-status capitalize rounded-2xl text-md">
          Đã sử dụng
        </button>
      </div>
...
export const gridStatusCheck = (props) => (
  <>
    {props.statusCheck === 1 ? (
      <span className="status-check status-check--true capitalize rounded-2xl text-md">Đã đối soát</span>
    ) : props.statusCheck === 0 ? (
      <span className="status-check status-check--false capitalize rounded-2xl text-md">Chưa đối soát</span>
    ) : (
      <></>
    )}
  </>
